  ...
  try {
    ...
    if (saves.length === 0) {
      ...
      for (const item of saves) {
        if (item.bests[item.leader - 1].score < lastBest.bests[item.leader - 1].score)
          lastBest = item;
        cnt += item.leader;
      }
...
app.put('/api/users/saveGames', async (req, res) => {
  try {
    ...
    const index = user.saveBoards.findIndex((item) => item.name === req.body.name);
    if (index !== -1) {
...
app.get('/api/users/leaders', async (req, res) => {
  try {
    const saves = await User.where('leader').gt(0).select('leader bests username -_id');
    if (saves.length === 0) {
      res.send(JSON.stringify(saves));
      return;
    }
